...
export function generateDiscoveryQuestions(problem: string): DiscoveryQuestion[] {
  ...
  return [
    ...
    { id: 'fix-looks-like', question: 'What would a useful fix look like?', reason: 'The smallest useful answer might not be software.', answerType: 'choice', choices: family ? ['One tap status', 'Shared family log', 'Reminders', 'A little bit of automation', 'Something that is not software'] : ['A quick reminder', 'A single status tracker', 'A shared list', 'A more automatic workflow', 'Something that is not software'], allowYouDecide: true },
  ];
}
